...
10. Хороший гайд для мобильного system design - https://github.com/iartr/mobile-system-design?tab=readme-ov-file
11. Мок собеседование - https://www.youtube.com/watch?v=9_8ShTF6aQA
12. Мок собеседование - https://www.youtube.com/watch?v=Wh5Ya6UFG1k
13. Мок собеседование - https://www.youtube.com/watch?v=_K-eupuDVEc
...
ACID — это набор принципов, которые гарантируют надежность транзакций в базе данных. Каждая транзакция (например, перевод денег, оформление заказа) должна быть целостной, правильной и не ломать систему, даже если вдруг что-то пойдет не так (например, отключится свет или упадёт сервер).

**Расшифровка ACID:**
* Atomicity (Атомарность) - Транзакция либо выполняется полностью, либо не выполняется вообще. Если что-то пошло не так, изменения откатываются.
Пример: Вы переводите деньги. Деньги либо списались и поступили получателю, либо вообще ничего не произошло.
* Consistency (Согласованность) - После завершения транзакции база остается в правильном (согласованном) состоянии. 
Пример: Если на счету было 1000₽ и вы перевели 200₽, то должно остаться 800₽, и у получателя — плюс 200₽. Никаких “потерянных” или “из воздуха взявшихся” денег.
* Isolation (Изолированность) - Транзакции не мешают друг другу, даже если идут одновременно.
Пример: Если два человека одновременно покупают последний билет, только один из них получит подтверждение, а второй — отказ.
* Durability (Надежность/Постоянство) - После завершения транзакции её результат сохраняется навсегда, даже при сбоях.
Пример: Если вы оплатили заказ и получили подтверждение, то даже если сервер упал — заказ не “исчезнет”.

ACID — это гарантия, что ваша база данных работает надежно, стабильно и предсказуемо. Особенно важно в банках, магазинах, финтехе и везде, где ошибка может стоить денег или доверия.
...
**Архитектура отдельных сервисов с подходом shared nothing** - это принцип проектирования распределённых систем, при котором каждый сервис (или узел) работает полностью автономно и не делит ресурсы с другими сервисами.
Основная идея в такой архитектуре - каждый сервис имеет свою память, свой диск, свою базу данных и независимые интерфейсы. Нет общих точек хранения или вычислений, сервисы взаимодействуют только через сеть (например, API-запросы). Это позволяет легко расширять систему, просто добавляя новые независимые сервисы.

Преимущества:
* Масштабируемость: горизонтальное масштабирование практически линейное — добавление сервиса добавляет новые ресурсы без влияния на существующие.
* Отказоустойчивость: отказ одного сервиса не приводит к сбоям других, изоляция ошибок повышает стабильность.
* Гибкость: каждый сервис можно обновлять, развивать и тестировать независимо, что упрощает поддержку и ускоряет релизы.

Недостатки и сложности:
* Управление консистентностью данных между сервисами требует продуманных механизмов синхронизации и согласования.
* Распределённые транзакции сложнее реализовать, так как каждый сервис автономен.
* Общая архитектура системы становится сложнее для проектирования и администрирования.
